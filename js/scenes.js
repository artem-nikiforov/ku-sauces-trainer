...
(function () {
  ...
  /* ВАЖНО про стиль. Векторные рисунки ниже — рабочая заглушка, а не цель.
     Эталон визуала курса — постер «Гость уходит с эмоцией»: полуреалистичная
     иллюстрация с живописной светотенью. Рукописным SVG такое не набирается,
     поэтому правильный путь — сгенерировать растр по промтам из
     assets/ASSETS.md §2 и включить его здесь одним флагом.

     Курс обязан работать и БЕЗ картинок: если файла нет, флаг остаётся false
     и рисуется вектор. Поэтому заглушки удалять нельзя даже после замены. */
  const PHOTOS = {
